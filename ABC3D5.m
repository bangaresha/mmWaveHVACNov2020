...
for jj=1:1:nNodes
    if z(jj) < 1.25 || z(jj) > 4.25
        nABC=nABC+1;
        ABC(nABC)=jj;
    else
        r0 = sqrt(x(jj)^2+y(jj)^2);
        if(abs(r0) > 2) %% && (z(jj) >= 0.5)
            nABC=nABC+1;
            ABC(nABC)=jj;
...
H_0 = 10; % Amplitude of incident electric field (V/m)
J = complex(0.,1.);
...
for e=1:1:nElements
    ...
    for i = 1:1:4
        be_tilde(e,i) = H_0*k_0*k_0*(1/mu_r(e) - eps_r(e))*V(e)/4;
    end
end
for e=1:nElements
...
for s = 1:1:nSegABC
    ...
    for ii = 1:1:3
        fun2 = @(ksi,ksj)bstilde1(ksi,ksj,ii,alpha,kappa,k_0,x1,x2,x3,y1,y2,y3,del(s),H_0);
...
figure
pdeplot3D(model,'ColorMapData',abs(Hz),'FaceAlpha',.3);

[xgrid, ygrid] = meshgrid(-1:0.01:1, -1:0.01:1);
Hzgrid = zeros(201,201); 
    ...
    for j = 1:1:201
        for e = 1:nElements
            ...
            if abs(V(e) - (a1p+a2p+a3p+a4p)) < 0.01*V(e)
                ...
                N1 = 1 - N2 - N3 - N4;
                Hzgrid(i,j) = N1*Hz(N(e,1))+N2*Hz(N(e,2))+N3*Hz(N(e,3))+N4*Hz(N(e,4));
            end
        end
    end
end
...
figure;
contourf(xgrid,ygrid,abs(Hzgrid));
xlabel('x (wavelengths)');
ylabel('y (wavelengths)');
title('Total Magnetic Field Contour Plot');
colorbar;

figure;
contourf(xgrid(10:20,10:20),ygrid(10:20,10:20),abs(Hzgrid(10:20,10:20)));
xlabel('x (wavelengths)');
ylabel('y (wavelengths)');
title('Total Magnetic Field Contour Plot');
colorbar;
...
for i = 1:length(zgridt)
    EzAtaDistance(Hz,x,y,z,N,nElements,nodeIndex,zgridt(i));
end
